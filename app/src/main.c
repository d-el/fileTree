...
//FREEDEL// traceLabel xTickTraceUserEvent;
static portBASE_TYPE xTraceRunning = pdTRUE;


static void taskprint(void *pPrm){
	char *str = (char*) pPrm;
	uint32_t cnt = 0;
...
int main( void ){
	...
	for(uint32_t i = 0; i < NUMBER_TSK; i++){
		char *taskName = malloc(configMAX_TASK_NAME_LEN * sizeof(char));
		sprintf(taskName, "Task%u", i);
		xTaskCreate(taskprint, taskName, configMINIMAL_STACK_SIZE, taskName, mainQUEUE_RECEIVE_TASK_PRIORITY, NULL);
	}

	//xTaskCreate(taskprint, "taskprint0", configMINIMAL_STACK_SIZE, "Task0", mainQUEUE_RECEIVE_TASK_PRIORITY, NULL);
	//xTaskCreate(taskprint, "taskprint0", configMINIMAL_STACK_SIZE, "Task1", mainQUEUE_RECEIVE_TASK_PRIORITY, NULL);
...
typedef struct CommunicationRcAls {
	uint8_t i;
} CommunicationRcAls;
...
void vApplicationTickHook( void )
{
	...
	#if ( mainCREATE_SIMPLE_BLINKY_DEMO_ONLY != 1 )
	{
		//FREEDEL// vFullDemoTickHookFunction();
	}
	...
	bursts. */
	//FREEDEL// vTraceUserEvent( xTickTraceUserEvent );
}
/*-----------------------------------------------------------*/
...
FILE* pxOutputFile;
	...
	{
		//FREEDEL// fwrite( RecorderDataPtr, sizeof( RecorderDataType ), 1, pxOutputFile );
		fclose( pxOutputFile );
